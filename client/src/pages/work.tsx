...
/* ── Animation styles (CSS-only keyframes) ─────────────────── */
const ANIM_CSS = `
  @keyframes wFadeUp   { from { opacity:0; transform:translateY(22px); }  to { opacity:1; transform:translateY(0); } }
  @keyframes wFadeIn   { from { opacity:0; }                              to { opacity:1; } }
  @keyframes wSlideR   { from { opacity:0; transform:translateX(-18px); } to { opacity:1; transform:translateX(0); } }
  @keyframes wSlideL   { from { opacity:0; transform:translateX(18px); }  to { opacity:1; transform:translateX(0); } }
  @keyframes wScale    { from { opacity:0; transform:scale(0.94); }        to { opacity:1; transform:scale(1); } }
  @keyframes wPulse    { 0%,100% { opacity:1; } 50% { opacity:0.4; } }
  .wv .d0 { animation: wFadeUp 0.55s ease-out 0.05s both; }
  .wv .d1 { animation: wFadeUp 0.55s ease-out 0.15s both; }
  .wv .d2 { animation: wFadeUp 0.55s ease-out 0.25s both; }
  .wv .d3 { animation: wFadeUp 0.55s ease-out 0.35s both; }
  .wv .d4 { animation: wFadeUp 0.55s ease-out 0.45s both; }
  .wv .dr { animation: wSlideL 0.55s ease-out 0.2s both; }
  .wv .dl { animation: wSlideR 0.55s ease-out 0.2s both; }
  .wv .ds { animation: wScale  0.5s  ease-out 0.1s both; }
  .wv-grid .d0 { animation: wFadeUp 0.45s ease-out 0.05s both; }
  .wv-grid .d1 { animation: wFadeUp 0.45s ease-out 0.12s both; }
  .wv-grid .d2 { animation: wFadeUp 0.45s ease-out 0.19s both; }
  .wv-grid .d3 { animation: wFadeUp 0.45s ease-out 0.26s both; }
  .wv-grid .d4 { animation: wFadeUp 0.45s ease-out 0.33s both; }
  .wv-grid .d5 { animation: wFadeUp 0.45s ease-out 0.40s both; }
  .wv-rows .d0 { animation: wFadeIn 0.4s ease-out 0.05s both; }
  .wv-rows .d1 { animation: wFadeIn 0.4s ease-out 0.15s both; }
  .wv-rows .d2 { animation: wFadeIn 0.4s ease-out 0.25s both; }
  .wv-rows .d3 { animation: wFadeIn 0.4s ease-out 0.35s both; }
`;
...
function Reveal({ children, className = "" }: { children: React.ReactNode; className?: string }) {
  ...
  return (
    <div ref={ref} className={visible ? `wv ${className}` : `opacity-0 ${className}`}>
      {children}
    </div>
  );
}
...
function RevealGrid({ children, className = "" }: { children: React.ReactNode; className?: string }) {
  const { ref, visible } = useInView();
  return (
    <div ref={ref} className={visible ? `wv-grid ${className}` : `opacity-0 ${className}`}>
      {children}
    </div>
  );
}

function RevealRows({ children, className = "" }: { children: React.ReactNode; className?: string }) {
  const { ref, visible } = useInView();
  return (
    <div ref={ref} className={visible ? `wv-rows ${className}` : `opacity-0 ${className}`}>
      {children}
    </div>
  );
}
            ...
            <RevealRows>
              {companies.map((c, i) => (
                <div key={i} className={`d${i} group`}>
                  ...
                  <div className="h-px bg-white/[0.07] group-hover:bg-white/[0.14] transition-colors" />
                </div>
              ))}
            </RevealRows>
          </div>
