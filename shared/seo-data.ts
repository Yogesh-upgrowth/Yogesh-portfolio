...
/**
 * Per-route Open Graph card, generated by script/gen-og-images.ts and committed
 * under client/public/og/. Every page used to share one image, so every share
 * on LinkedIn or X rendered an identical card.
 */
export function ogImagePath(routePath: string): string {
  if (routePath === "/") return "/og/home.png";
  return `/og${routePath}.png`;
}

export function ogImageUrl(routePath: string): string {
  return `${SITE_URL}${ogImagePath(routePath)}`;
}
