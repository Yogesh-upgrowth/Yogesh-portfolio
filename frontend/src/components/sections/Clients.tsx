  ...
  return (
    ...
    >
      ...
      <div className="relative container px-4 md:px-6 mx-auto max-w-5xl">
        ...
        <div
          ref={headRef}
          className={`flex flex-col md:flex-row md:items-end md:justify-between gap-4 md:gap-5 mb-8 md:mb-14 reveal ${headVisible ? "in-view" : ""}`}
        >
          <div>
            {/* Status badge */}
            <div className="inline-flex items-center gap-2 mb-3 md:mb-5 px-3 py-1.5 rounded-full border border-primary/25 bg-primary/5">
          ...
          <p className="text-muted-foreground max-w-xs leading-relaxed text-[13px] md:text-base">
        ...
        >
          {clientTypes.map((item, i) => (
            <div
              key={item.id}
              className="client-card group relative flex flex-col p-4 md:p-6 rounded-2xl bg-background border border-border/70 hover:border-primary/40 hover:shadow-[0_0_24px_rgba(37,99,235,0.07)] active:scale-[0.99] transition-all duration-300 overflow-hidden"
            ...
            >
              ...
              <div className="flex items-center justify-between mb-3 md:mb-4">
                <span className="text-[10px] md:text-[11px] font-bold tracking-widest text-muted-foreground/50 group-hover:text-primary/50 transition-colors font-mono">
                  {item.id}
                </span>
                <span className="inline-flex items-center gap-1 md:gap-1.5 text-[9px] md:text-[11px] font-bold uppercase tracking-wider px-2 md:px-2.5 py-0.5 md:py-1 rounded-full border border-primary/20 bg-primary/5 text-primary/70 group-hover:bg-primary/10 group-hover:text-primary transition-all">
              ...
              {/* Main label */}
              <p className="font-serif font-bold text-foreground text-sm md:text-[17px] leading-snug mb-2 md:mb-3 group-hover:text-primary transition-colors duration-200">
                {item.label}
              </p>

              {/* Detail */}
              <p className="text-muted-foreground text-[12px] md:text-sm leading-relaxed">
                {item.detail}
              </p>
            </div>
          ))}
          ...
          <Link href="/contact" className="group block">
            <div className="relative flex flex-col items-center justify-center text-center p-5 md:p-8 rounded-2xl border-2 border-dashed border-primary/25 bg-primary/[0.025] hover:border-primary/50 hover:bg-primary/[0.05] active:scale-[0.99] transition-all duration-300 min-h-[130px] md:min-h-[160px] overflow-hidden">
              {/* Pulsing centre glow */}
              <div
              ...
              <div className="relative">
                <div className="inline-flex items-center gap-1.5 md:gap-2 mb-2 md:mb-3">
                  ...
                  <span className="text-[9px] md:text-xs font-bold tracking-widest uppercase text-primary/60">
                    Open Slot
                  </span>
                </div>
                <p className="font-serif font-bold text-base md:text-xl text-foreground mb-0.5 md:mb-1 group-hover:text-primary transition-colors">
                  Your Company?
                </p>
                <p className="text-muted-foreground text-[12px] md:text-sm mb-3 md:mb-4">Let's see if we're a fit</p>
                <span className="inline-flex items-center gap-1.5 text-[13px] md:text-sm font-semibold text-primary group-hover:gap-2 transition-all">
                  Start a conversation <ArrowRight className="h-3.5 w-3.5 md:h-4 md:w-4" />
                </span>
              </div>
            </div>
          </Link>
